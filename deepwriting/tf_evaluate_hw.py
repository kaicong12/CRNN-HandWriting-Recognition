...
run_reconstruction = True # Reconstruct reference samples and save reconstruction results.
...
conditional_texts = ["This is a synthetic sentence."]
# Indices of reference style samples from validation split.
reference_sample_ids = [107, 696]
...
def plot_eval_details(data_dict, sample, save_dir, save_name):
    ...
    if plot_latent_vars and 'p_mu' in data_dict:
        ...
        if save_plots:
            imwrite(os.path.join(save_dir, save_name + '_normal.png'), plot_img)
    ...
    if plot_latent_vars and 'p_pi' in data_dict:
        ...
        if save_plots:
            imwrite(os.path.join(save_dir, save_name + '_pi.png'), plot_img)

    if plot_eoc and 'out_eoc' in data_dict:
        plot_img = plot_and_get_image(np.squeeze(data_dict['out_eoc']))
        imwrite(os.path.join(save_dir, save_name + '_eoc.png'), plot_img)
    ...
    if 'gmm_mu' in data_dict:
        ...
        if save_plots:
            imwrite(os.path.join(save_dir, 'gmm_mu.png'), gmm_mu_img)
            imwrite(os.path.join(save_dir, 'gmm_sigma.png'), gmm_sigma_img)
