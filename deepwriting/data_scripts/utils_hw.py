...
def get_text(int_labels, eoc_labels, sow_labels, label_encoder, in_raw_threshold=2, eoc_threshold=0.5):
    """
    ...
    sow_indices = np.where((sow_labels > eoc_threshold) == 1)[0].tolist()
    sow_indices.append(len(eoc_labels))
    ...
    for idx in range(len(int_labels)-1):
        if (int_labels[idx] == int_labels[idx+1]) and (int_labels[idx] != 0):
            ...
            if abs(eoc_idx - idx) < min_char_distance/3.0:
                if idx > sow_indices[next_space_index]:
                    chars.append(" ")
                    next_space_index += 1
